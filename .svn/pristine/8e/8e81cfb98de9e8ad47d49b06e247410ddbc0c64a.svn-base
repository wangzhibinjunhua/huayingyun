...
public class BleManager {
    ...
    public boolean connect(final String address) {
        if (!isConnecting) {
            ...
            if (mBluetoothAdapter == null || address == null) {
                if (DBT)
                    Log.w("wutl", "BluetoothAdapter not initialized or unspecified address.");
                return false;
            }
            ...
            if (mDevice == null) {
                if (DBT)
                    Log.w("wutl", "Device not found.  Unable to connect.");
                return false;
            }
    ...
    public void disconnect() {
        if (mBluetoothAdapter == null || mBluetoothGatt == null) {
            if (DBT)
                Log.w("wutl", "BluetoothAdapter not initialized");
            return;
        }
        // 断开与远程设备的GATT连接。
    ...
    public void readDescriptor(BluetoothGattCharacteristic characteristic, String descriptorUUID) {
        ...
        if (mBluetoothAdapter == null || mBluetoothGatt == null) {
            if (DBT)
                Log.w("wutl", "BluetoothAdapter not initialized");
            if (DBT)
                Log.w("wutl", "mBluetoothGatt is " + mBluetoothGatt);
            return;
        }
