...
public class DataTab extends RelativeLayout implements RadioGroup.OnCheckedChangeListener, View.OnClickListener {
    ...
    private GlucoseView view_glucose;
    ...
    private void init(final Context context, final int measureType) {
        ...
        registerEvent();
        if (measure_type == DataBean.TYPE_BLOOD_PRESSURE_MONITOR || measure_type == DataBean.TYPE_GLUCOSE_METER || measure_type == DataBean.TYPE_BLE_EAR || measure_type == DataBean.TYPE_BLE_ECG||measure_type == DataBean.TYPE_BLE_OPD) {
...
//                if (measure_type==DataBean.TYPE_GLUCOSE_METER){
//                    if (null == view_glucose) {
//                        view_glucose = new GlucoseView(context);
//                    }
//                    defaultIndex = 1;
//                    // view_measurement.destroy();
//                    showOrGoneBtnRight(VISIBLE);
//                    view_measurement.scanBleDevice(false);
//                    changeView(view_glucose);
//                }else {
                if (null == view_data) {
