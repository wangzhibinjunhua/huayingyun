...
public class DeviceView extends LinearLayout {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                if (TApplication.user.isLogin()) {
                    ...
                    if (deviceBean != null) {
                        ...
                        MEASURE_TYPE = deviceBean.getType();
                        if (MEASURE_TYPE == DataBean.TYPE_BLE_BTEMP) {
                            bundle.putInt(KEY_MEASURE_TYPE, MEASURE_TYPE);
                            IntentUtil.gotoActivity(context, TempActivity.class, bundle);
                            return;
                        }
                        bundle.putInt(KEY_MEASURE_TYPE, MEASURE_TYPE);
    ...
    private void addBindDevice() {
        ...
        bean = new DeviceBean();
        bean.setDeviceName(context.getString(R.string.bluetooth_temp));
        bean.setUrl(R.mipmap.icon_temp_paste);
        bean.setType(DataBean.TYPE_BLE_BTEMP);
        bindedDevices.add(bean);
